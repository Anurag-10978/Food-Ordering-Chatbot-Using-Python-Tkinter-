...
def get_response(user_input):
    ...
    if "menu" in user_input:
        ...
        for category, items in menu.items():
            ...
            msg += "\n"
        msg += "Type 'order pizza, burger' or 'add biryani and naan' to start ordering."
        return msg
    ...
    if added_items:
        total = calculate_total()
        added_str = ", ".join([i.title() for i in added_items])
        return f"{added_str} added to your order.\nCurrent Total: ₹{total}"
    ...
    responses = [
        ...
        "You can order multiple items like 'order pizza, burger, fries'.",
...
chat_window = scrolledtext.ScrolledText(
    root, wrap=tk.WORD, font=("Helvetica", 12), bg="white", fg="black"
)
...
entry_box = tk.Entry(root, textvariable=user_input, font=("Helvetica", 12), width=32, bg="#FFF8E1")
entry_box.place(x=20, y=510, height=40)

# Buttons
tk.Button(root, text="Send", command=send_message, font=("Helvetica", 11, "bold"),
          bg="#4CAF50", fg="white", width=10).place(x=470, y=510, height=40)

tk.Button(root, text="Clear Chat",
          ...
          font=("Helvetica", 11, "bold"), bg="#f44336", fg="white", width=12).place(x=230, y=560, height=30)

tk.Button(root, text="Total Bill", command=show_total_bill, font=("Helvetica", 11, "bold"),
          bg="#2196F3", fg="white", width=12).place(x=20, y=560, height=30)
...
chat_window.insert(tk.END,
    ...
    "You can order multiple items like 'order pizza, fries, coke'.\n\n"
)
chat_window.config(state=tk.DISABLED)
